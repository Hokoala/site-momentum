...
export default function LoginPage() {
  ...
  // Rediriger si déjà connecté (après tous les hooks)
  useEffect(() => {
    if (session) {
      router.push("/admin");
    }
        ...
        <div className="bg-black/80 backdrop-blur-md border border-white/10 p-8 relative group">
          ...
          {!registrationsOpen && !isSignUp && (
            <div className="bg-yellow-500/10 border border-yellow-500/30 text-yellow-500 p-2 mb-6 text-[10px] font-mono uppercase tracking-wide flex items-center justify-center gap-2">
              <LockKeyhole className="w-3 h-3" /> Inscriptions système
              verrouillées
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {isSignUp && (
              <div className="space-y-2">
                <label
                  htmlFor="name"
                  className="text-[10px] font-mono text-white/50 uppercase tracking-widest pl-1"
                >
                  Identifiant
                </label>
            ...
            >
              ...
              <span className="relative flex items-center gap-2">
                {loading ? (
                  ...
                  <>
                    {isSignUp ? "Initier Protocole" : "Connexion Système"}{" "}
